...
export function sanitizeErrorForClient(
  ...
  if (
    /ABORTED|Transaction|lock timeout|PERMISSION_DENIED|FAILED_PRECONDITION|auth\/|firestore|FirebaseError|Firebase/i.test(
      s
    ) ||
    /NOT_FOUND|ALREADY_EXISTS|UNAVAILABLE|RESOURCE_EXHAUSTED|DEADLINE_EXCEEDED|INTERNAL/i.test(
      s
    ) ||
